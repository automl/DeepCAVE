...
from pimp.evaluator.forward_selection import AbstractEvaluator, ForwardSelector
...
class Importance(object):
    def __init__(
        self,
        scenario_file: Union[None, str] = None,
        scenario: Union[None, Scenario] = None,
        runhistory_file: Union[str, None] = None,
        runhistory: Union[None, RunHistory] = None,
        traj_file: Union[None, List[str]] = None,
        incumbent: Union[None, Configuration] = None,
        seed: int = 12345,
        parameters_to_evaluate: int = -1,
        margin: Union[None, float] = None,
        save_folder: str = "PIMP",
        impute_censored: bool = False,
        max_sample_size: int = -1,
        fANOVA_cut_at_default=False,
        ...
        if 0 < max_sample_size < len(self.X):
            ...
            if self.best_dir:
                ...
                self._load_incumbent(
                    glob.glob(os.path.join(self.best_dir, "*traj_aclib2*"), recursive=True)[0],
                    None,
                    incumbent,
                )
    ...
    def _preprocess(self, runhistory):
        ...
        for c_id, config in tqdm(
            enumerate(configs), ascii=True, desc="Completed: ", total=len(configs)
        ):
            config = impute_inactive_values(config).get_array()
            X_prime.append(config)
            X_non_hyper.append(config)
            y_prime.append(
                self.model.predict_marginalized_over_instances(np.array([X_prime[-1]]))[0].flatten()
            )
    ...
    def _setup_scenario(
        self, scenario: Union[None, Scenario], scenario_file: Union[None, str], save_folder: str
    ) -> None:
        """
        ...
        if scenario is not None:
            ...
            raise Exception(
                "Either a scenario has to be given or a file to load it from! Both were set to None!"
            )
    ...
    def _load_incumbent(self, traj_file, runhistory_file, incumbent, predict_best=True) -> None:
        ...
        if incumbent is not None:
            ...
            for traj_ in traj_files:
                ...
                incumbents[-1].extend(
                    self._model.predict_marginalized_over_instances(
            ...
            raise Exception(
                "No method specified to load an incumbent. Either give the incumbent directly or specify "
    ...
    def _load_runhist(self, runhistory, runhistory_file) -> None:
        ...
        if runhistory is not None:
            ...
            raise Exception(
                "Either a runhistory or files to load them from have to be specified! Both were set to "
                "None!"
            )
        self.logger.info(
            "Combined number of Runhistory data points: %d" % len(self.runhistory.data)
        )
    ...
    def _read_traj_file(self, fn):
        ...
        with open(fn) as fp:
            ...
            fileformat = (
                "aclib2" if isinstance(json.loads(fp.readline())["incumbent"], list) else "alljson"
            )

        if fileformat == "aclib2":
            self.logger.info(
                "Format is 'aclib2'. This format has issues with recovering configurations properly. We "
    ...
    def model(self, model_short_name="urfi"):
        if model_short_name not in ["urfi", "rfi"]:
            raise ValueError(
                "Specified model %s does not exist or not supported!" % model_short_name
            )
            ...
            if not self._preprocessed:
                self.types, self.bounds = self._get_types(
    ...
    def _convert_data(self, fit=True) -> None:  # From Marius
        ...
        if self.scenario.run_obj == "runtime":
            ...
            threshold = np.log10(self.scenario.cutoff * self.scenario.par_factor)
        ...
        if not self.impute:
            self.logger.info(
                "Thus the size of X might be smaller than the datapoints in the RunHistory"
            )
        if fit:
    ...
    def evaluate_scenario(
        self, methods, save_folder=None, plot_pyplot=True, plot_bokeh=False
    ) -> Union[
        ...
        for rnd, method in enumerate(methods):
            ...
            if save_folder and plot_pyplot:
                self.evaluator.plot_result(
                    os.path.join(save_folder, self.evaluator.name.lower()), show=False
                )
            if save_folder and plot_bokeh:
                self.evaluator.plot_bokeh(
                    os.path.join(save_folder, self.evaluator.name.lower() + "_bokeh")
                )
            if load:
