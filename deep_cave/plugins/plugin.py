...
class Plugin(Layout):
    ...
    def _register_callbacks(self):
        ...
        def plugin_output_update(n_clicks, *user_specified_inputs_list):
            if n_clicks is None:
                ...
                outputs = self.process(rm.get_selected_run(), **user_specified_inputs)
            ...
            def plugin_input_update(*user_specified_inputs_list):
                ...
                if init:
                    ...
                    if user_specified_inputs is None:
                        user_specified_inputs = self._load_input(rm.get_selected_run())
                    ...
                    user_dependencies_inputs = self._load_dependency_input(rm.get_selected_run(), **user_specified_inputs)
    ...
    def get_layout(self):
        ...
        return [
            ...
            # Input
            html.Div(id=f'{self.id()}-input', className="shadow p-3 mb-3 bg-white rounded", children=self.get_input_layout()),

            # Output
            html.Div(id=f'{self.id()}-output', className="shadow p-3 bg-white rounded", children=self.get_output_layout())
        ]

    def process(self, run, **kwargs):
        """
        ...
        return self._load_output(run, **kwargs)

    @abstractmethod
    def _load_output(self, **kwargs):
        pass
    ...
    def get_input_layout(self):
        ...
        input_layout += [
            dbc.Button(
                ...
                style={"display": "none"} if not self.update_on_changes() else {}
            )
        ]
