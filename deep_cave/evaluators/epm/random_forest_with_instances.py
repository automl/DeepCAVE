...
from deep_cave.evaluators.epm.forest import Forest
...
class RandomForestWithInstances(Forest):
    ...
    def _predict(self, X: np.ndarray,
        ...
        if self.log_y:
            ...
            preds_as_array = np.zeros(
                (X.shape[0], self.forest_options.num_trees, third_dimension)) * np.NaN
            ...
            preds_as_array = np.log(np.nanmean(
                np.exp(preds_as_array), axis=2) + VERY_SMALL_NUMBER)
